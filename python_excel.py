...
def read_excel_pufa(file_name):
	...
	l_pufa = ['浦发银行',count] #银行名和账号放进一个临时list，后面把所有信息一起作为一个list，append进all_info，后面取值方便
...
def write_excel(all_infos):
	...
	for i in range(len(all_infos)):
		sheet.write_merge(2, 2, 14+3*i, 16+3*i, all_infos[i][0],style_title)
		...
		for j in range(len(all_infos[i]) - 2):
			...
			sheet.write(5+j + info_count, 5, all_infos[i][0].replace('银行',''),style_content)
